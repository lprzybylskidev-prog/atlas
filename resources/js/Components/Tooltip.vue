...
<template>
    <span class="group/tooltip relative inline-flex" :class="{ 'w-full': fullWidth }">
        <slot />
        <span
            v-if="!disabled"
            role="tooltip"
            class="pointer-events-none absolute z-50 rounded-md bg-zinc-950 px-2 py-1 text-xs font-medium whitespace-nowrap text-white opacity-0 shadow-lg transition-[opacity,transform] duration-300 ease-in-out group-hover/tooltip:opacity-100 group-focus-within/tooltip:opacity-100 dark:bg-zinc-100 dark:text-zinc-950"
            :class="[
                placement === 'right'
