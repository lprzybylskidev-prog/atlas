...
<template>
    <span class="group/tooltip relative inline-flex max-w-full" :class="{ 'w-full min-w-0': fullWidth }">
        ...
        <span
            v-if="!disabled"
            role="tooltip"
            class="pointer-events-none absolute z-50 select-none rounded-md bg-zinc-950 px-2 py-1 text-xs font-medium whitespace-normal text-white opacity-0 shadow-lg transition-[opacity,transform] duration-300 ease-in-out break-words group-hover/tooltip:opacity-100 group-focus-within/tooltip:opacity-100 dark:bg-zinc-100 dark:text-zinc-950"
            :class="[
                wide ? 'max-w-[48rem]' : 'max-w-80',
                placement === 'right'
