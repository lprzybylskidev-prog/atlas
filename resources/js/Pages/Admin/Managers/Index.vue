...
interface RelationshipHistoryRow extends Record<string, unknown> {
    publicId: string;
    managerName: string;
    reportName: string;
    validFrom: string;
...
const historyRows = computed<RelationshipHistoryRow[]>(() =>
    props.history.map((relationship) => ({
        ...
        reason: relationship.validTo === null ? relationship.reason : (relationship.endReason ?? relationship.reason),
    })),
);
const historyColumns: DataTableColumn<RelationshipHistoryRow>[] = [
    { key: 'managerName', label: 'Manager' },
    { key: 'reportName', label: 'Direct report' },
    { key: 'validFrom', label: 'Valid from', format: 'datetime' },
    { key: 'validTo', label: 'Valid to', format: 'datetime' },
    ...
    <AdminLayout :title="t('pages.admin.managers.title')" :title-icon="IconSitemap">
        <PageStack>
            <section class="grid gap-3 lg:grid-cols-[minmax(20rem,1.35fr)_repeat(3,minmax(10rem,0.55fr))]">
                <SurfaceCard title="Team selector" :icon="IconUsers">
            ...
            <div class="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)]">
                <SurfaceCard title="Current hierarchy" :icon="IconGitBranch">
                    <template #actions>
                        <StatusBadge :value="flatTree.length > 0" true-label="Active" false-label="Empty" />
                    </template>

                    <UiState v-if="flatTree.length === 0" variant="empty" title="No active hierarchy for this team." />
                ...
                <aside class="space-y-5">
                    <SurfaceCard title="New relationship" :icon="IconArrowRight">
                        <AtlasForm class="space-y-4" :processing="assignForm.processing" @submit="submitRelationship">
                            <FormSelect
                    ...
                    <SurfaceCard title="Head managers" :icon="IconUserStar">
                        <AtlasForm class="space-y-4" :processing="headForm.processing" @submit="addHeadManager">
                        ...
                        >
                            <UiState v-if="headManagers.length === 0" variant="empty" title="No head managers set." size="compact" />
                            <template v-else>
            ...
            <SurfaceCard title="Active relationships" :icon="IconGitBranch">
                <template #actions>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ relationships.length }} active</p>
                </template>
                <UiState v-if="relationships.length === 0" variant="empty" title="No active relationships." size="compact" />
                <div v-else class="divide-y divide-zinc-200 rounded-lg border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">
                    <article v-for="relationship in relationships" :key="relationship.publicId" class="space-y-4 p-4">
                        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(10rem,0.45fr)_minmax(10rem,0.45fr)_auto]">
                            <div class="min-w-0">
                                <p class="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">Manager</p>
                                <p class="mt-1 truncate font-medium text-zinc-950 dark:text-zinc-50">{{ relationship.managerName }}</p>
                                <p class="mt-1 break-all text-xs text-zinc-500 dark:text-zinc-400">{{ relationship.managerEmail }}</p>
                            </div>
                            <div class="min-w-0">
                                <p class="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">Direct report</p>
                                <p class="mt-1 truncate font-medium text-zinc-950 dark:text-zinc-50">{{ relationship.reportName }}</p>
                                <p class="mt-1 break-all text-xs text-zinc-500 dark:text-zinc-400">{{ relationship.reportEmail }}</p>
                            </div>
                            <div>
                                <p class="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">Valid from</p>
                                <p class="mt-1 text-sm text-zinc-700 dark:text-zinc-200">
                                    {{ formatTimestamp(relationship.validFrom, 'en') }}
                                </p>
                            </div>
                            <div>
                                <p class="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">Impact</p>
                                <p class="mt-1 text-sm text-zinc-700 dark:text-zinc-200">
                                    {{ relationship.endPreview.affectedReportPublicIds.length }} affected
                                </p>
                            </div>
                            <div class="flex items-start justify-end">
                                <FormButton type="button" tone="neutral" :icon="IconChevronDown" @click="chooseRelationship(relationship)">
                                    End
                                </FormButton>
                            </div>
                        </div>
                        ...
                        <AtlasForm
                            v-if="selectedRelationship === relationship.publicId"
                            class="grid gap-3 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900/60 md:grid-cols-[12rem_minmax(0,1fr)_auto]"
