...
| `employee`              | Pracownik / Pracownicy                              | Pracownicy / Pracownicy / Pracownik                                            | Employee / Employees                                       | Employees / Employees / Employee                                                          |
| `manager`               | Manager / Managerowie                               | Managerowie / Managerowie / Manager                                            | Manager / Managers                                         | Managers / Managers / Manager                                                             |
| `head_manager`          | Główny manager / Główni managerowie                 | Główni managerowie / Główni managerowie / Główny manager                       | Head Manager / Head Managers                               | Head Managers / Head Managers / Head Manager                                              |
...
- `resources/js/Localization/translationUsageInventory.ts` inventories literal frontend translation calls and translation-key properties across the production Vue/TypeScript tree. `App\Shared\Presentation\Localization\AtlasTranslationUsageInventory` performs the corresponding backend, breadcrumb, Blade, flash, and Inertia-key inventory. Both inventories are checked independently against Polish and English catalogs, and mutation tests remove a known used key from each locale to prove the guards fail.
- Every template-literal translation lookup belongs to a finite family in `resources/js/Localization/dynamicTranslationFamilies.ts`. The registry enumerates accepted domain values independently of the locale files; an unregistered dynamic expression or a missing family member fails Vitest. Namespace-wide scanner exclusions and catalog-derived wildcard acceptance are forbidden.
- The runtime `[translation:<key>]` value is diagnostic only. Shared fallback formatters must recognize it with `isMissingTranslation`; route-backed Playwright sweeps reject this marker and visible untranslated `pages.`, `navigation.`, `actions.`, `auth.`, `breadcrumbs.`, `datatable.`, and `filters.` product copy in both locales.
