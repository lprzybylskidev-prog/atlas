...
DataTable formats that render shared status or severity badges use one centralized presentation classification. Their cell-content wrapper must remain overflow-visible and must not apply text truncation; badge components expose the shared `data-ui="status-badge"` marker so the browser guard can reject any clipping ancestor between a badge and its table cell. Adding a badge-rendering format requires extending this shared classification and its exhaustive format test, not a page-specific CSS exception.

The Team Structure view is the canonical team-context membership and hierarchy editor. Adding a member is a primary action placed before the hierarchy. The primary surface groups every active member exactly once as a Head Manager, Manager, or Employee, and the section headings own the role context without repeated role badges on member cards. Head Managers show whole-Team scope, Managers show direct-report and parent counts, and Employees show Manager counts. Expandable Manager cards show only their direct reports in a responsive card grid; membership actions remain secondary, and a member's own Managers are not repeated in details. Desktop drag-and-drop adds one Manager relationship through the audited backend mutation and preserves any existing Managers; the member card does not duplicate that interaction with an assignment button. Structural-role changes use one reasoned modal with a backend impact preview. Removing a Manager assignment or ending membership uses a focused destructive confirmation with target identity and a required reason; destructive-reason inputs never remain permanently visible in member cards. Field validation renders beside its modal control, while domain and operation failures remain visibly in the open modal and leave accepted state unchanged. Effective-dated relationship and membership history and mandatory Audit evidence remain owner-controlled backend concerns. The old tree/reparent editor and its dead reparent contract are removed. Team Edit may link to this view but must not recreate membership mutation controls. Team Edit member authorization is explicitly read-only: roles, direct permissions, policy overrides, current source, and the effective preview remain visible without Save, reason, or removal controls. User Edit is the canonical authorization-mutation surface. Its direct-permission selector presents role-derived grants as checked, disabled effective access with localized granting-role labels while retaining the persisted direct-grant model separately. Browser coverage exercises the desktop lifecycle, successful and rejected destructive actions, plus mobile and keyboard disclosure behavior, and the permanent foundation guard rejects restoring the duplicate Team Edit workflow, permanent destructive-reason inputs, or a separate Managers area.
