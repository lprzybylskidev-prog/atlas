...
The Team Structure view is the canonical team-context membership and hierarchy editor. Its primary surface groups every active member exactly once as a Head Manager, Manager, or Employee. Head Managers show whole-Team scope, Managers show direct-report and parent counts, and Employees show Manager counts. Expandable cards keep relationship and membership detail secondary. Desktop drag-and-drop and the accessible keyboard/mobile action both add one Manager relationship through the same audited backend mutation; they preserve any existing Managers. Structural-role changes use one reasoned modal with a backend impact preview. Removing a Manager assignment or ending membership uses a focused destructive confirmation with target identity and a required reason; destructive-reason inputs never remain permanently visible in member cards. Field validation renders beside its modal control, while domain and operation failures remain visibly in the open modal and leave accepted state unchanged. Effective-dated relationship and membership history and mandatory Audit evidence remain owner-controlled backend concerns. The old tree/reparent editor and its dead reparent contract are removed. Team Edit may link to this view but must not recreate membership mutation controls. Team Edit member authorization is explicitly read-only: roles, direct permissions, policy overrides, current source, and the effective preview remain visible without Save, reason, or removal controls. User Edit is the canonical authorization-mutation surface. Its direct-permission selector presents role-derived grants as checked, disabled effective access with localized granting-role labels while retaining the persisted direct-grant model separately. Browser coverage exercises the desktop lifecycle, successful and rejected destructive actions, plus mobile and keyboard disclosure behavior, and the permanent foundation guard rejects restoring the duplicate Team Edit workflow, permanent destructive-reason inputs, or a separate Managers area.
