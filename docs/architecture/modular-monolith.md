...
Shared Inertia props are composed through explicit Presentation contributors registered with the `atlas.inertia_shared_data` tag. Route visibility candidates are registered with `atlas.inertia_route_availability` and filtered through backend permissions by shared Presentation composition. Contributors own only their module's data, return deterministic dot-path keys, and duplicate shared prop paths fail fast. Global Inertia middleware remains a thin adapter that calls the registry and must not know module internals or module-owned tables.
