...
`composer analyse` runs PHPStan/Larastan at the configured maximum practical level through `tools/quality/run-phpstan.sh`. The script discovers targets deterministically from `phpstan.neon`, expands modules automatically, includes global `app` directories such as `Http` and `Providers`, and verifies that every configured PHP file is covered by the public command. It analyses application/runtime code in one batch, current test areas in bounded batches, and the larger Unit tree by its owned areas. This preserves one public maximum-level gate while avoiding the native PHP 8.5 exit-139 failure caused by loading the entire application and test tree into one PHPStan process.

The public PHPStan script disables PHPStan parallel workers by default because the current PHPStan/Larastan/PHP runtime combination can terminate a worker with exit code 139 during otherwise clean analysis. Set `PHPSTAN_DISABLE_PARALLEL=0` only while diagnosing that runtime behavior. Keep the public target discovery, bounded batches, and coverage verification intact; do not work around the runtime fault by skipping PHPStan or bypassing Git hooks.
