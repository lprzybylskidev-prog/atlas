...
`composer check` remains the standard local gate. `composer check:foundation` is the full foundation gate and runs the standard gate, full Playwright, and the production runtime smoke sequentially. Phase 34 later owns the distinct final release gate after the completed Phase 30 Authorization/Team Structure repair, Phase 31 internal Chat/Calendar/Calls/Meetings communication scope, Phase 32 Diagnostics/User Bug Reports, and Phase 33 production deployment, backup, restore, and rollback are implemented.
