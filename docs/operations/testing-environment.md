...
`composer check` remains the standard local gate. `composer check:foundation` is the full foundation gate and runs the standard gate, full Playwright, and the production runtime smoke sequentially. Phase 34 owns the database query-efficiency and HTTP route audit after Phase 33 production deployment, backup, restore, and rollback are implemented. Phase 35 then owns the distinct final release gate after the completed Phase 30 Authorization/Team Structure repair, Phase 31 internal Chat/Calendar/Calls/Meetings communication scope, Phase 32 Diagnostics/User Bug Reports, Phase 33 deployment/recovery, and Phase 34 query audit.
