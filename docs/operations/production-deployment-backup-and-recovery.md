...
Canonical production topology and operational procedures. This document complements the binding [Phase 40 deployment roadmap](../roadmap/phase-40-deployment-backup-rollback.md).
...
Phase 40 provides one canonical production installation workflow through an interactive installer after all accepted first-base-release foundations through Phase 39 are complete. It builds on the existing production images, runtime configuration validation, readiness, runtime smoke, queues, scheduler, Files, ClamAV, PDF, search, security, privacy, Reverb, self-hosted LiveKit, Egress, runtime Settings/secrets, OIDC, Service Accounts, API, webhooks, and Integration Events rather than replacing them.
...
Application and nginx production images are reproducible and smoke-tested; production runtime secrets are externalized; broad `DB_SEARCH_PATH` masking is removed; and non-HTTP services remain private. Phase 40 adds the private host installation workflow, optional TLS configuration, durable local Files storage, deployment, backup, restore, and rollback on that verified runtime foundation.
...
Interactive configuration collects deployment/bootstrap values only: installation/company name, hostname or internal address, technical fallback timezone where required, internal network binding, HTTP/TLS mode, supplied certificate paths, persistent Files path, PostgreSQL storage, backup location/retention/schedule, required SMTP configuration, and first-administrator identity where applicable. Values Atlas can safely own are generated rather than requested. Team business timezones and OIDC/runtime policy are Admin-managed after bootstrap, and the installer does not configure Sentry.
...
Production validation remains fail-fast: debug mode is disabled, the deployed marker is true, technical fallback timezone and database search path match the Atlas contract, typed settings are valid, required values are non-empty, and the fake Files scanner is forbidden. Phase 40 owns host secret provisioning and rotation without weakening these Phase 28 rules. Recovery preserves deployment-owned encryption material required to decrypt restored Admin-managed runtime secrets without embedding it in the protected backup.
...
This is a safe buildable foundation, not the complete production backup system. Phase 40 adds scheduling, retention, Files coverage, off-host strategy, restore, application verification, monitoring, and operational runbooks.
...
Backup existence alone is insufficient. Phase 40 requires a real restore drill and verification of representative restored application data. Restore procedures must state how database and Files artifacts relate so the operator does not unknowingly restore an inconsistent pair.
