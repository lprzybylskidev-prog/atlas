...
- `App\Shared\Application\Teams\Contracts\TeamMembershipChangeParticipant` is the neutral optional-consumer port invoked by Teams membership mutations. The deployed Chat contribution synchronizes its system Team conversation in the same transaction; an installation without a contributor keeps the Teams workflow in a safe reduced mode.
...
- adding a Manager relationship through desktop drag-and-drop, with an effective date and reason while preserving existing Managers;
...
Current state: Teams owns membership and the integrated team structure editor. The editor places member creation before the hierarchy, groups members once by explicit structural role, gives Head Managers whole-Team scope, and adds Manager relationships through desktop drag-and-drop. Member cards rely on their section headings instead of repeating role badges; expanded Manager cards show their direct reports in a responsive grid and do not repeat the member's own Managers. Structural roles change through one reasoned impact-preview workflow. Adding a Manager preserves existing Manager relationships. The earlier tree/reparent-centric editor and its unused atomic reparent contract were removed after the additive relationship workflow became canonical. Phase 28 removed the duplicated separate Admin Managers area after route, permission, UI, DAG, audit, concurrency, and legacy-reference coverage was moved to the Teams surface.
