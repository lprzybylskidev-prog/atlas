...
The Admin privacy and retention area is available at `/admin/privacy-retention`, with `/admin/privacy-retention/legal-holds` for legal-hold and retention blocker management. It is separate from Audit / Security: Audit remains immutable diagnostic evidence, while Privacy and retention owns deletion, anonymization, legal-hold, retention, and controlled-copy readiness workflows. The current surface shows protection requirements, high-risk dry-run impact previews, lifecycle participant coverage, retention-controlled areas, legal-hold creation, and backend-applied filters through the shared Admin DataTable foundation. It does not execute hard delete or anonymization until the full Phase 26 high-risk workflow framework is implemented.
