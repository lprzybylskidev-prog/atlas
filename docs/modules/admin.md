...
The authenticated shell resolves Admin primary navigation, mobile navigation, TopBar mode switching, subsection links, active state, canonical breadcrumb labels, and backend route availability through the single typed frontend navigation registry. Desktop and mobile therefore expose the same accepted Admin routes. A persistent security-context strip identifies the active team and active Admin mode and reports whether fresh high-risk authorization is available; it also surfaces impersonation and offline state when applicable.
...
Manager hierarchy administration belongs to Teams and opens from Team Edit at `/admin/teams/{team}/structure`. The integrated editor groups active members once as Head Managers, Managers, or Employees; shows role-specific relationship counts and expandable detail; adds Manager relationships through desktop drag-and-drop or the equivalent keyboard/mobile action; and changes structural roles through one reasoned impact-preview modal. Relationship creation is additive and preserves existing Managers. The backend rejects cycles, self-management, duplicates, Head Manager participation, and stale writes, protects the last Head Manager, and records security-sensitive Teams audit events. Atlas has no separate Managers Admin navigation or CRUD area.
...
The failed-job section shows queue/connection/job/exception summaries and expandable payload and exception details for authorized operators. Handled-by user labels are resolved through Identity-owned lookup contracts, not direct Identity table joins. Administrators may retry one failed job after confirmation, retry selected rows through a DataTable bulk action, mark one failed job as handled, or mark selected rows as handled. Handled failed jobs stop contributing to Admin failed-job dashboard warnings and operational alert thresholds while remaining visible in diagnostics when the handled filter is selected. Retry and handling actions are audited as security-sensitive queue operations. Admin does not expose arbitrary `queue:retry all`, range retry, queue clearing, failed-job flushing, shell access, or arbitrary command execution.
...
The Admin privacy and retention area is available at `/admin/privacy-retention`, with `/admin/privacy-retention/legal-holds` for legal-hold creation, reasoned release, and retention blocker management. It is separate from Audit / Security: Audit remains immutable diagnostic evidence, while Privacy and retention owns deletion, anonymization, legal-hold, retention, and controlled-copy workflows. Executable previews expose a permission-gated final hard-delete/anonymization form with exact typed confirmation; the backend revalidates the impact snapshot and blockers and commits participant execution plus mandatory audit atomically.
...
Phase 25 rebuilt the Admin presentation around the shared Atlas view standard. Current primary Admin areas are Dashboard/System Status, Users, Teams (including team structure and manager hierarchy), Roles, Permissions, Szablony, Modules, Queues, Managed Processes, Files, Privacy and retention, Search, Integrations, Feature Flags, Rate Limits, Application Logs, Audit bezpieczeństwa, and package-owned Pulse/Telescope diagnostics links. Imports do not have a standalone Admin route; import execution visibility belongs to Managed Processes, where import-linked runs keep their context and detail links.
...
Module-owned issue counts shown in System Status are collected through the shared `ModuleOperationalDiagnostics` contributor registry. Shared Admin code owns composition and display only; Files, Identity, Integrations, Managed Processes, and Imports own the persistence queries behind their diagnostic issue rows.
