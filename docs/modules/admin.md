...
Manager hierarchy administration belongs to Teams and opens from Team Edit at `/admin/teams/{team}/structure`. The integrated editor shows the hierarchy below the selected member, supports multi-report assignment with effective dates and impact preview, ends relationships, updates head-manager status, rejects cycles/self-management/stale writes, protects the last head manager, and records security-sensitive Teams audit events. Atlas has no separate Managers Admin navigation or CRUD area.
