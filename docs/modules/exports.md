...
- Application DataTable export requests are accepted through `exports.data-table` at `/exports/data-table`. This endpoint uses the same provider registry, snapshot factory, artifact lifecycle, and Core Exports authorization path as Admin DataTable exports, but does not require Admin mode. It is intended for ordinary application report tables that already enforce their own module permissions and provider authorization.
