...
- [Phase 33 — Foundation extension-point, duplication, and consumer audit](phase-33-foundation-extension-points-and-duplication-audit.md)
- [Phase 34 — Module-owned routing, navigation, breadcrumbs, and application surfaces](phase-34-module-owned-routing-navigation-breadcrumbs.md)
- [Phase 35 — Runtime Settings, localized reference data, Team timezones, and Business Calendars](phase-35-runtime-settings-localized-reference-data-team-timezones-business-calendars.md)
- [Phase 36 — Reusable data integrity and business-support primitives](phase-36-reusable-data-integrity-business-primitives.md)
- [Phase 37 — Enterprise OIDC identities and authentication-method separation](phase-37-enterprise-oidc-external-identities.md)
- [Phase 38 — Atlas API, Service Accounts, Integration Events, OpenAPI, and webhooks](phase-38-api-service-accounts-integration-events-webhooks.md)
- [Phase 39 — Developer extension contract, module documentation, and MDK](phase-39-developer-extension-contract-and-mdk.md)
- [Phase 40 — Private production deployment, installer, backup, restore, and rollback](phase-40-deployment-backup-rollback.md)
- [Phase 41 — Database Query Efficiency and Route Performance Audit](phase-41-database-query-efficiency-and-route-performance-audit.md) must be complete before this phase begins.
...
- For localization, Phase 42 must prove that Polish and English are complete in rendered UI, backend-provided props, validation messages, flash/toast messages, notification text, breadcrumbs, forms, tables, and operational helper copy. It must include negative assertions against accidental English user-facing copy in Polish mode except for allowed technical diagnostic values.
- For messaging, Phase 42 must prove ownership and noise limits for user feedback. Workflows such as exports, imports, retries, scans, rebuilds, managed processes, and integrations must not create duplicate flashes, toast storms, or competing terminal notifications.
...
- [ ] Verify shared Core Calendar Month/Week/Day/Agenda views, private personal events, Team-timezone-pinned recurrence, reminders, and privacy-preserving Free/Busy.
...
- [ ] Verify no Atlas Sentry package/integration/configuration/environment/runtime/browser/source-map/release/deployment hook remains after Phase 32, no replacement external monitor exists, and generic shared secret redaction works.
- [ ] Verify Phase 33 audit closure, backlog freeze, assigned findings, and documented module public boundaries.
- [ ] Verify Phase 34 module-owned routes/navigation/breadcrumbs, surface semantics, natural URLs, uniqueness, ModuleGate, privacy, and legacy central-file cleanup.
- [ ] Verify Phase 35 Settings/env ownership, write-only secrets, localized values/dictionaries, immutable codes, soft-delete history, Team timezones, and named Business Calendars.
- [ ] Verify Phase 36 locking/conflicts, Change Reason, effective ranges, provenance, safe references, and concurrent global/Team sequences.
- [ ] Verify Phase 37 OIDC linking, provider matrix, modes, recovery, authentication-method separation, secrets, Audit, and failure behavior in the required browsers.
- [ ] Verify Phase 38 Integration Events/Outbox, `/api/v1`, OpenAPI, Service Accounts, API-only permissions, mandatory token expiry/replacement, idempotency, and signed webhooks.
- [ ] Verify Phase 39 extension documentation, architecture guards, minimal Application/Optional generator, collision safety, and generated-module checks.
