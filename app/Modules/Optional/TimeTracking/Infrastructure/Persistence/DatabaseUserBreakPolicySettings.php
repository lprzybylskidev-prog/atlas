...
final readonly class DatabaseUserBreakPolicySettings implements UserBreakPolicySettings
{
    ...
    {
        return [
            $this->users->internalIdForPublicId($userPublicId) ?? 0,
            $this->teams->internalIdForPublicId($teamPublicId) ?? 0,
        ];
    }

    private function assignmentId(int $userId, int $teamId): int
    {
        return $this->teams->activeAssignmentInternalIdForUserTeam($userId, $teamId) ?? 0;
    }
