...
final readonly class DatabaseSettingsStore implements SettingsStore
{
    ...
    public function getGlobal(GlobalSettingKey $key): mixed
    {
        return $this->remember(
            ...
            fn (): mixed => $this->settingValue('settings_global_values', ['key' => $key->value], $this->defaults->global($key)),
        );
    }

    public function putGlobal(GlobalSettingKey $key, mixed $value): void
    {
        $this->upsert('settings_global_values', ['key' => $key->value], $this->validator->validate($key, $value));
    ...
    public function getTeam(int $teamId, TeamSettingKey $key): mixed
    {
        return $this->remember(
            ...
            fn (): mixed => $this->settingValue('settings_team_values', ['team_id' => $teamId, 'key' => $key->value], $this->defaults->team($key)),
        );
    }

    public function putTeam(int $teamId, TeamSettingKey $key, mixed $value): void
    {
        $this->upsert('settings_team_values', ['team_id' => $teamId, 'key' => $key->value], $this->validator->validate($key, $value));
    ...
    public function getUser(int $userId, UserSettingKey $key): mixed
    {
        return $this->remember(
            ...
            fn (): mixed => $this->settingValue('settings_user_values', ['user_id' => $userId, 'key' => $key->value], $this->defaults->user($key)),
        );
    }

    public function putUser(int $userId, UserSettingKey $key, mixed $value): void
    {
        $this->upsert('settings_user_values', ['user_id' => $userId, 'key' => $key->value], $this->validator->validate($key, $value));
    ...
    public function getSecurity(SecuritySettingKey $key): mixed
    {
        return $this->remember(
            ...
            fn (): mixed => $this->settingValue('settings_security_values', ['key' => $key->value], $this->defaults->security($key)),
        );
    }
    ...
    public function putSecurity(SecuritySettingKey $key, mixed $value, ?string $actorPublicId = null, ?string $reason = null): void
    {
        ...
        $this->upsert('settings_security_values', ['key' => $key->value], $validated);
