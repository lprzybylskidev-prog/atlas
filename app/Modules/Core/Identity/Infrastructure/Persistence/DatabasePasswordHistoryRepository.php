...
final class DatabasePasswordHistoryRepository implements PasswordHistoryRepository
{
    ...
    {
        $hashes = DB::table(IdentityDatabaseTable::USER_PASSWORD_HISTORIES)
    ...
    public function record(int $userId, string $passwordHash, int $limit): void
    {
        ...
        $retainedIds = DB::table(IdentityDatabaseTable::USER_PASSWORD_HISTORIES)
            ->where('user_id', $userId)
            ->orderByDesc('created_at')
            ->orderByDesc('id')
            ->limit($limit)
        ...
        DB::table(IdentityDatabaseTable::USER_PASSWORD_HISTORIES)
            ->where('user_id', $userId)
